...
def moved(facility, to, items):
    ...
    if not safe_combo(facility.floors[facility.elevator]):
        return False
    facility.floors[to] |= items
    if not safe_combo(facility.floors[to]):
        return False
    ...
    for e2 in [e-1, e+1]:
        ...
        for n in [1, 2]:
            for items in itertools.combinations(facility.floors[facility.elevator], n):
                if not safe_combo(facility.floors[facility.elevator] - set(items)):
                    continue
                if not safe_combo(facility.floors[e2] | set(items)):
                    continue
...
def play(inp):
    ...
    while future:
        ...
        if dist > furthest:
            print(f'{dist=}, {len(future)=}, {rejects=}')
            furthest = dist
        #print(dist)
        #print(f)
        #print()
...
print(play(real_input)) # => 61  BUT VERY SLOW
